...
> - **Toast feedback untuk aksi (tambahan pasca-Fase 7, permintaan user):** sistem toast ringan baru `src/components/ui/toast.tsx` — `<Toaster>` (provider + viewport) di-mount di root layout `src/app/layout.tsx`, dipakai lewat `useToast()` dengan helper `toast.promise(promise, { loading, success, error })` yang menampilkan lifecycle **loading → sukses/error** dalam satu toast (loading persist sampai selesai; sukses auto-dismiss 4s; error auto-dismiss 8s + tombol tutup; tombol tutup tidak muncul saat loading). Terpasang di semua aksi: simpan/hapus kelas, kitab (simpan + aktif/nonaktif), santri (simpan + aktif/nonaktif), akun ustadz/wali (buat + aktif/nonaktif), relasi wali↔santri, simpan nilai ustadz, login, logout. Banner error/sukses inline lama yang duplikat dengan toast dihapus; **banner error untuk data-load (mount/effect) dipertahankan inline** — bukan aksi user, tidak perlu toast. Style pakai token DESIGN.md (`bg-surface`, `text-success`/`text-danger`, animasi `animate-toast-in` di `globals.css`), tanpa library toast. Verified `npm run lint` + `npm run build` hijau.
...
- [x] **12.5** Helper (`src/lib/absensi-stats.ts`): `getKegiatanTemplates(kegiatanId)` + tipe `KegiatanTemplateItem` (`updatedAt` ISO); `getSesiAbsensi`/`SesiAbsensiData` diperluas dengan `totalSesi` (jumlah sesi kegiatan) agar konteks laporan lengkap tanpa query tambahan.
- [x] **12.6** API (`requireApiRole(["admin","ustadz"])` — wali 403): `GET/POST /api/kegiatan/[id]/template` (list urut `created_at`; 404 bila kegiatan tidak ada), `PATCH/DELETE /api/kegiatan/[id]/template/[templateId]` (404 bila template bukan milik kegiatan; 400 validasi zod).
- [x] **12.7** UI: `TemplateManager` (client) sebagai seksi "Template Laporan" di `KegiatanDetailManager` (admin & ustadz) — daftar + Tambah/Edit/Hapus, editor `Dialog` existing (Nama + Textarea + contekan variabel klik-sisip di kursor + peringatan typo). `LaporanCard` (client) di halaman sesi admin & ustadz — dropdown template → preview `<pre>` scroll (`whitespace-pre-wrap`) → Salin (`navigator.clipboard`, sukses toast; gagal → toast + teks terseleksi untuk salin manual); empty state + link ke detail kegiatan.
- [x] **12.8** Sinkronkan docs: `SCHEMA.md` (tabel + diagram), `ARCHITECTURE.md` (§3 route/komponen + §4), `IMPLEMENTATION.md` (fase ini).
